    ...
    def sns_scatter(self, df, target, related, reg_summary, hue=None, N=3):
        ...
        if q == 0 or (q == 1 and r == 0):
            ...
            for idx, column in enumerate(related):
                ...
                axes[idx].text(1, -max(df[target])//10*1, "+1 follower: {}".format(reg_summary[target][idx]), fontdict=fdic, bbox=boxdic)
            ...
            for idx, column in enumerate(related):
                ...
                axes[x, y].text(1, -max(df[target])//10*1, "+1 follower: {}".format(reg_summary[target][idx]), fontdict=fdic, bbox=boxdic)
