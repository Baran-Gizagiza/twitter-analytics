...
if df[0]:
    ...
    # パラメータ呼び出し
    st.sidebar.header("1. Parameter(+1 followers)")
    corr_rank = Views().corr_rank()
    st.sidebar.header("")

    st.sidebar.header("2. Parameter(your column)")
    ...
    st.sidebar.header("3. Parameter(Good Tweet)")
    ...
    # 単回帰分析
    st.header('1-2. Single Regression Analytics')
    ...
    if len(related_month) == 0:
        st.warning("There are no related columns for {}. Decrease 'corr_rank' and try again!".format(target))
    else:
        ...
        single_detail = st.checkbox("Detail Single Regression")
        if single_detail:
            st.dataframe(reg_summary)

        # 重回帰分析
        st.header('1-3. Multi Regression Analytics')
        ...
        if analyze:
            ...
            # 関連するカラムがない場合、corr_rankを下げることで関連カラムを取得できます。
            if len(related) == 0:
                st.warning("There are no related columns for {}. Decrease 'corr_rank' and try again!".format(target_2))
            else:
                st.success("There are related columns for {}".format(target_2))
                PreProcessing().histgram_related(df_quan, related)

                st.header('2-4. Single Regression Analytics')
                ...
                st.header('2-5. Multi Regression Analytics')
